...
import { Plus, Search, Settings2, ExternalLink, Database } from "lucide-react";
          ...
          <Plus className="h-4 w-4" />
            ...
            {/* <div className="flex items-center justify-between p-4 border-b">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-lg font-semibold">Vector Databases</h1>
                </div>
                <Button
                    onClick={() => setAddDialogOpen(true)}
                    size="sm"
            ...
            <div className="flex-1 overflow-auto">
                {filteredDatabases.map((db) => (
                    <div 
                        key={db.name}
                        className={`
                            group px-3 py-2 hover:bg-gray-50 cursor-pointer border-l-2
                            ${selectedDb === db.name ? 'border-l-blue-500 bg-blue-50/50' : 'border-l-transparent'}
                        `}
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="flex items-center justify-between">
                                    <div className="truncate">
                                        ...
                                        <span className="text-xs text-gray-500">{formatNumber(db.documentCount)} docs</span>
                                    </div>
                                ...
                                <div className="flex items-center gap-2 text-xs text-gray-500">
                                    <Badge 
                                        variant="outline" 
                                        className={`${getStatusColor(db.status)} px-1.5 py-0 h-4 text-[10px] font-normal`}
                                    >
                                        {db.status.charAt(0).toUpperCase() + db.status.slice(1)}
                                    </Badge>
                                    <span className="truncate text-xs text-gray-500">{db.description}</span>
                                </div>
                            </div>
                        </div>
                    </div>
