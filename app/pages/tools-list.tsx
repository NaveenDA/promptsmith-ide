...
const ToolsList = () => {
    ...
    const getProgressColor = (status: "pending" | "running" | "completed" | "failed") => {
        switch (status) {
            case "completed":
                return "bg-green-500";
            case "running":
                                ...
                                <div className="space-y-2 min-w-0">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-600 mt-0.5">
                                            Enable tools locally by following
                                            these steps:
                                        </p>
                                    </div>
                                        ...
                                        <div className="space-y-1.5">
                                            <div className="flex items-center justify-between text-[10px]">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className={getStatusColor(tool.securityTests.status)}>
                                                        Security Tests: {tool.securityTests.completed}/{tool.securityTests.total}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <Zap className="w-3 h-3 text-gray-400" />
                                                    <span className="text-gray-500">
                                                        {tool.securityTests.details.reduce((acc, curr) => acc + curr.passed, 0)} passed
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="flex-1">
                                                <Progress 
                                                    value={(tool.securityTests.completed / tool.securityTests.total) * 100} 
                                                    className="h-1"
                                                    indicatorClassName={getProgressColor(tool.securityTests.status)}
                                                />
                                            </div>
                                        </div>
                                    )}
